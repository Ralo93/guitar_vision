...
class ChordTrainer:
    def __init__(self, model, criterion, optimizer, device, save_path=r'\models\best_model2.pth', threshold=0.5):
        self.model = model
...
class PitchTrainer:
    def __init__(self, model, criterion, optimizer, device, save_path=r'models\best_model2.pth'):
        self.model = model
        self.criterion = criterion
        self.optimizer = optimizer
        self.device = device
        ...
        self.train_losses = []
        self.val_losses = []
        self.class_names = ["Am", "Bb", "C", "Dm", "Em", "F", "G"]  # 7 pitch classes
    ...
    def evaluate(self, dataloader):
        ...
        with torch.no_grad():
            for inputs, labels in dataloader:
                inputs, labels = inputs.to(self.device), labels.to(self.device)
                outputs = self.model(inputs)
                predicted = torch.argmax(outputs, dim=1)  # For multiclass, we use argmax to get the predicted class
                all_preds.extend(predicted.cpu().numpy())
                all_labels.extend(labels.cpu().numpy())
